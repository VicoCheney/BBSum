        ...
        with torch.no_grad():
            ...
            for i in range(len(bufs)):
                ...
                for j in range((bs - 1) // max_bs + 1):
                    ...
                    result = self.summarizer(input_ids=ids[l:r], attention_mask=attn_masks[l:r], labels=label[l:r], decoder_attention_mask=decoder_attention_mask[l:r])
                    result = result.loss
                ...
                if len(losses) != 0:
                    ...
                    t = 0
                    for blk in bufs[i]:
                        if losses_delta[t] >= self.config.levelup_threshold and blk.relevance < 3:
